...
import { Coins, Edit, Camera } from "lucide-react";
...
const StudentProfile: React.FC<StudentProfileProps> = ({
  ...
  const saveProfileChanges = () => {
    try {
      ...
      if (studentIndex !== -1) {
        students[studentIndex].displayName = displayName;
        localStorage.setItem("students", JSON.stringify(students));
        
        // Update local storage for current student if it's their own profile
        if (isOwnProfile) {
      ...
      toast(t("error-updating-profile"));
    }
  };
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-4">
            <div className="space-y-4">
              <div>
                <p className="text-sm font-medium mb-2">{t("display-name")}</p>
                <Input 
                  value={displayName} 
