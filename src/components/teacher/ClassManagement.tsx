...
const ClassManagement: React.FC<ClassManagementProps> = ({
  ...
  const handleCreateClass = () => {
    ...
    const newClassData: Class = {
      ...
      schoolId,
      teacherId,
      // This teacher becomes the creator/owner
      students: [],
  ...
  const handleSelectClass = (cls: Class) => {
    ...
    const savedStudents = localStorage.getItem("students");
    const parsedStudents = savedStudents ? JSON.parse(savedStudents) : [];
    const classStudents = parsedStudents.filter((student: Student) => cls.students?.includes(student.id));
    setStudents(classStudents);

    // Determine available students (not already in this class)
    const studentsNotInClass = allStudents.filter(student => !cls.students?.includes(student.id));
  ...
  const handleSearchStudents = () => {
    ...
    try {
      ...
      const results = parsedStudents.filter((student: Student) => student.username.toLowerCase().includes(searchUsername.toLowerCase()));

      // Filter out students already in this class
      const filteredResults = selectedClass ? results.filter((student: Student) => !selectedClass.students.includes(student.id)) : results;
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold">
            {selectedClass ? <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={handleBackToClasses}>
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                {selectedClass.name}
                {!isClassCreator && <Badge variant="outline" className="ml-2 gap-1">
        ...
        {!selectedClass && <Button onClick={() => setIsNewClassDialogOpen(true)}>
            <Plus className="h-4 w-4 mr-1" />
            {t("add-class")}
          </Button>}
        
        {selectedClass && isClassCreator && <div className="flex gap-2">
      ...
      {!selectedClass ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {classes.map(cls => {
        const isCreator = cls.teacherId === teacherId || isAdminUser;
        return <Card key={cls.id} className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <CardTitle>{cls.name}</CardTitle>
                    {!isCreator && <Badge variant="outline" className="gap-1">
                        <Lock className="h-3 w-3" />
                        {t("view-only")}
                      </Badge>}
                  </div>
                ...
                <CardFooter className="flex justify-between">
                  <Button variant="default" onClick={() => handleSelectClass(cls)}>
                    {isCreator ? t("manage") : t("view")}
                  </Button>
                  {isCreator && <Button variant="ghost" size="sm" onClick={() => handleDeleteClass(cls.id)} className="text-red-500">
          ...
          {classes.length === 0 && <Card className="col-span-full border-dashed">
            ...
            {students.length > 0 ? <Table>
                ...
                <TableBody>
                  {students.map(student => <TableRow key={student.id}>
                      <TableCell>
                        <span className="cursor-pointer hover:text-blue-600 hover:underline" onClick={() => handleViewStudentDetail(student.id)}>
                          {student.displayName}
                        </span>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          {isClassCreator && <Button variant="ghost" size="sm" onClick={() => handleRemoveStudentFromClass(student.id)} className="text-red-500 h-8">
                ...
                {isClassCreator && <div className="flex flex-col sm:flex-row justify-center gap-3">
                    <Button onClick={openAddByUsernameDialog}>
                      <UserPlus className="h-4 w-4 mr-1" />
                      {t("add-by-username")}
                    </Button>
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div>
              ...
              <Input id="className" value={newClass.name} onChange={e => setNewClass({
              ...newClass,
              name: e.target.value
            ...
            <div>
              ...
              <Input id="classDescription" value={newClass.description} onChange={e => setNewClass({
              ...newClass,
      ...
      <Dialog open={isStudentDialogOpen} onOpenChange={setIsStudentDialogOpen}>
        <DialogContent>
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {availableStudents.length > 0 ? <div className="space-y-4">
                {availableStudents.map(student => <div key={student.id} className="flex items-center justify-between border-b pb-2">
                    <div>
              ...
              </div> : <p className="text-center py-6">{t("no-available-students")}</p>}
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div className="flex gap-2">
              <Input value={searchUsername} onChange={e => setSearchUsername(e.target.value)} placeholder={t("enter-username")} />
              <Button onClick={handleSearchStudents} disabled={isSearchingStudent || !searchUsername.trim()}>
                <Search className="h-4 w-4" />
              </Button>
            </div>
            
            {searchResults.length > 0 ? <div className="max-h-[300px] overflow-y-auto space-y-2">
                {searchResults.map(student => <div key={student.id} className="border rounded-md p-3 flex justify-between items-center">
                    <div>
                      <p className="font-medium">{student.displayName}</p>
                      <p className="text-sm text-gray-500">@{student.username}</p>
                    </div>
              ...
              </div> : searchUsername.trim() && !isSearchingStudent ? <p className="text-center py-4 text-gray-500">{t("no-students-found")}</p> : null}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showStudentPokemon} onOpenChange={setShowStudentPokemon}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-4">
            {studentPokemon.length > 0 ? <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {studentPokemon.map((pokemon, index) => <div key={index} className="text-center">
                    <img src={pokemon.image || "/placeholder.svg"} alt={pokemon.name} className="w-20 h-20 mx-auto object-contain" />
                    <p className="text-sm font-medium mt-2">{pokemon.name}</p>
                  </div>)}
              </div> : <p className="text-center py-6">{t("no-pokemon-yet")}</p>}
          </div>
        </DialogContent>
      </Dialog>
    </div>;
