...
const ClassDetailsPage: React.FC = () => {
  ...
  const fetchStudentDetails = async (studentIds: string[]) => {
    ...
    try {
      ...
      let matchingStudents = localStudents.filter((s: any) => studentIds.includes(s.id));
      try {
  ...
  return <div className="min-h-screen bg-blue-50">
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="flex items-center mb-6">
          ...
          <Button variant="outline" size="icon" onClick={handleRefresh} className="ml-auto h-9 w-9 bg-white">
        ...
        {loading ? <div className="flex justify-center items-center h-64">
          ...
          </div> : classData ? <div className="space-y-6">
            <Card className="bg-white shadow-md border-t-4 border-t-pokemon-red">
              ...
              <CardContent>
                <div className="space-y-4">
                  {classData.description && <p className="text-gray-600 italic">{classData.description}</p>}
            ...
            <Card className="bg-white shadow-md">
              <CardHeader>
                <CardTitle>
                  <div className="flex items-center justify-between w-full">
                    ...
                    {isTeacherOrAdmin && <Button className="bg-pokemon-red hover:bg-red-600 text-white" onClick={() => setIsStudentDialogOpen(true)}>
                        {t("add-students")}
                      </Button>}
                  </div>
                </CardTitle>
              ...
              <CardContent>
                {loadingStudents ? <div className="py-8 flex justify-center">
                    <Loader2 className="h-6 w-6 animate-spin text-primary" />
                  </div> : students.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {students.map(student => <div key={student.id} className="p-4 rounded-lg border bg-gray-50 hover:bg-gray-100 transition-colors">
                        <div className="flex items-center">
                          <div className="h-10 w-10 bg-blue-100 text-blue-800 rounded-full flex items-center justify-center mr-3">
                            {(student.display_name || student.username || '').substring(0, 1).toUpperCase()}
                          </div>
                          <div>
          ...
          </div> : <Card className="bg-red-50 border-red-200">
      ...
      {classId && <StudentsList classId={classId} open={isStudentDialogOpen} onOpenChange={setIsStudentDialogOpen} onStudentsAdded={handleAddStudents} />}
    </div>;
};
